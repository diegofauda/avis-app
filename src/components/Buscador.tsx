...
export default function Buscador({
  items, value, onChange, placeholder, required, nuevoTexto,
}: {
  items: Item[];
  value: string;
  ...
  placeholder?: string;
  required?: boolean;
  nuevoTexto?: (v: string) => string;
}) {
  const [open, setOpen] = useState(false);
  const q = value.trim().toLowerCase();
  const filtered = q ? items.filter((c) => c.nombre.toLowerCase().includes(q)) : items;
  const shown = filtered.slice(0, 50);
  const exact = items.some((c) => c.nombre.toLowerCase() === q);
  ...
  const elegir = (nombre: string) => { onChange(nombre); setOpen(false); };
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute z-20 mt-1 max-h-64 w-full overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-lg">
          {value.trim() && !exact && nuevoTexto && (
            <button type="button" onMouseDown={(e) => { e.preventDefault(); elegir(value.trim()); }}
          ...
          {shown.length === 0 && !value.trim() && <div className="px-3 py-3 text-sm text-slate-400">Escribí para buscar…</div>}
          {shown.map((c) => (
            <button key={c.id} type="button" onMouseDown={(e) => { e.preventDefault(); elegir(c.nombre); }}
